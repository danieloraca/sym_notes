...
The MCP user must already exist. The server exposes owner-scoped tools for listing, searching, reading, creating, updating, moving, archiving, and restoring notes, plus listing and creating folders. `notes_attach` attaches base64-encoded binary files up to 10 MB to an existing note, and note responses include attachment metadata. The server also exposes `files_save` for saving text files under each authenticated user's private shared files directory; paths are relative, parent directories are created automatically, and overwriting requires an explicit flag. Folder creation supports an optional owned parent folder and rejects duplicate names within the same parent. The server intentionally does not expose permanent note deletion.
